...
public class CustomCssSelectorTypeDeserializer extends AbstractDataTypeDeserializer<CssSelectorType> {

    public CustomCssSelectorTypeDeserializer() {
        super(CssSelectorType.class);
    }

    @Override
    protected Object getValue(JsonParser p, DeserializationContext ctxt) throws IOException {
        return p.getValueAsString().replaceAll("#.*", "").trim();
    }
}